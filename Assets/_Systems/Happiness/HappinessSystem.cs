...
public class HappinessSystem : MonoBehaviour
{
    ...
    private void CheckHomeless()
    {
        ...
        var CPU = map.HappinessTotal.CPUs.Find((cpu) => { return cpu.name == "�޼ҿɹ�"; });

        if (map.MainData.totalCapacity < map.MainData.People && CPU.name == null)
        {
            map.HappinessTotal.AddCPU(new SolidMiddleware<Int>.CPU { name = "�޼ҿɹ�", Addition = new Int(-15) });
        }
    ...
    {
        var CPU = map.HappinessTotal.CPUs.Find((cpu) => { return cpu.name == "��ɫ��Դ"; });
        if (Mathf.Abs(PowerSystem.greenPowerRatio - 1f) < 0.01f && CPU.name == null)
            map.HappinessTotal.AddCPU(new SolidMiddleware<Int>.CPU { name = "��ɫ��Դ", Addition = new Int(15) });
    ...
    private void CheckMoney()
    {
        ...
        var CPU = map.HappinessTotal.CPUs.Find((cpu) => { return cpu.name == "��ԣũ��"; });
        if ((map.FarmProfitTotal.currentValue.m_value + OtherProfit) >= 25 && CPU.name == null)
            map.HappinessTotal.AddCPU(new SolidMiddleware<Int>.CPU { name = "��ԣũ��", Addition = new Int(10) });
